...
Matches escaped LIKE against `full_name`, `university`, and `major`. Returns projected fields: `user_id`, `full_name`, `avatar_url`, `university`, `major`, `headline` (server-computed from workplace or major/university; ordered by `full_name` ASC). Empty result when search is blank or shorter than 2 chars.
