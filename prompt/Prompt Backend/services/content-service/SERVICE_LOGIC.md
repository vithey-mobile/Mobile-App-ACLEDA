...
| Flow | Logic |
| --- | --- |
| Feed | Return posts from followed users plus self, ordered by newest, paginated. Enrich with batched counts + author/media resolution. **Only when `search` query param is absent.** |
...
- Home feed uses `GET /posts` (no `search`, no `sort` param).
