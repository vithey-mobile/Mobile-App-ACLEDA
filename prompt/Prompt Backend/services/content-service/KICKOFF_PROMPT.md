...
- Media URLs from file-service only — store references, not binaries.
- Publish RabbitMQ events for notification-service on social actions.

## Definition of done

Runnable Spring Boot on port 8084 implementing every endpoint in `SERVICE_PROMPT.md`, with tests per root `COMMON_CONTEXT.md`.
