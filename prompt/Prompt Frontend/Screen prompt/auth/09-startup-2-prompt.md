...
> **Pair with** `08-startup-1-prompt.md` and `10-startup-3-prompt.md`.  
...
| Field | Value |
| --- | --- |
| Screen ID | `09` / legacy `03F` / Start Up step 2 of 3 |
...
Same as `08-startup-1-prompt.md`:

- Fixed **AppBar** with **logo** + **Skip** (not “Vithey StartUp” text)
- Fixed **bottom** Back Â· dots Â· Next
...
- Configurable min/max (temp default â‰¥1, â‰¤5 if no product rule)
...
- Announce selection count / max; â‰¥44×44; light/dark contrast; not color-only.
