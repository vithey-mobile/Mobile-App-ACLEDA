# 03 - Auth / Sign In Screen Prompt (Version 2)

Build the **Auth screen (Sign In panel)** for the Vithey App in Flutter.

> **Do not modify** `03-auth-prompt.md`. This version-2 file is the new UI direction.  
> **Pair with** `04-register-prompt-version-2.md` — Sign In and Sign Up live on **one Auth screen** and morph between each other (grow/shrink).  
...
| Sibling panel | Sign Up / Register (same screen — see `04-register-prompt-version-2.md`) |
...
Scaffold
└── Stack
    ├── AuthTealBackdrop()                 // FIXED full-back teal
    └── Column
        ├── Expanded → logo (teal auto height)
        └── AuthMovingWaveSheet            // MOVING with panel toggle only
            ├── wave band ≈ 10% screen     // light teal + white curly edge
            └── white body                 // HUGS form content (Figma hug)
```

### Background (Auth-specific)

1. **Fixed — teal only**
   - Full-back teal (`AppColors.authHeaderTeal` ≈ `#2FC5C1`).
   - Height is **auto**: fills whatever space remains above the white sheet.
   - Rough visual guide ≈ 30% when Sign In content is short; grows/shrinks as white hugs taller/shorter forms.
   - Does not move when switching Sign In ↔ Sign Up.

2. **Moving — light teal wave band + white body**
   - Wave band ≈ **10% of screen height** (light teal + white curly edge together).
...
- **No vertical scroll** and **no horizontal swipe**.
- Switch Sign In ↔ Sign Up **only** with footer toggle buttons.
- White sheet **hugs content**; teal above is **auto** (remaining space).
- Wave band ≈ **10%** of screen height (adaptive %).
...
- `login()` / `loginWithGoogle()` — same contracts as v1.
- Panel index / `PageController` for Sign In ↔ Sign Up.
...
- `02-onboarding-prompt-version-2.md` (wave style)
- `04-register-prompt-version-2.md`
