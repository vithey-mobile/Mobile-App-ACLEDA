# 07 - Start Up 1 / Skills Selection Prompt (Version 2)

Build **Start Up 1**, the first post-registration profile-setup screen for the Vithey App in Flutter.

> **Do not modify** `07-startup-1-prompt.md`. This version-2 file is the new direction.  
> **Pair with** `08-startup-2-prompt-version-2.md` and `09-startup-3-prompt-version-2.md`.  
...
| Field | Value |
| --- | --- |
| Screen ID | `07` (v2) / legacy `03E` / Start Up step 1 of 3 |
...
This is **not** pre-auth marketing onboarding (`02-onboarding-prompt-version-2.md`); it is authenticated profile setup after account creation / first social login.
...
- Both directions must feel natural (Next → content slides left / next in from right; Back → reverse). Prefer the same clipped-lane pattern used for Auth register Part 1 ↔ Part 2 (slide stays inside content padding, not full-bleed phone edge).

### 2. AppBar

| Current (v1) | Version 2 |
| --- | --- |
...
5. **Bottom nav** (fixed shell) — Back · dots (step 1 active) · Next
...
- Require ≥1 skill before Next unless product allows empty.
...
- `Wrap` for chips; ≥16 side padding on narrow screens.
...
Same draft / repository boundary as v1 (`StartupProfileDraft.skillIds`, `saveSkills`). Do not invent production URLs. Local draft for recovery only; clear after final submit / confirmed Skip.

## Accessibility

- Semantics for logo AppBar, Skip, chips, Back, dots (**Step 1 of 3**), Next.
- Announce selected state + count; ≥44×44 targets; contrast in light/dark; don’t rely on color alone.
...
- `03-auth-prompt-version-2.md` / register v2 (entry into startup)
