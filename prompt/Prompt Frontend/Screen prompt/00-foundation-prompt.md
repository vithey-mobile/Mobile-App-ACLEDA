...
Build the base Flutter project skeleton for **Vithey App**.

- **Repo folder:** `vithey_app/`
- **pubspec.yaml name:** `aub_connect_app` (ACLEDA competition package name)
...
- Asset folders: `assets/images/`, `assets/images/brand/`, `assets/images/splash/`, `assets/icons/`, `assets/fonts/`
- **Copy official logo:** `Prompt Frontend/screen image/auth/logo app.png` → `assets/images/brand/logo_app.png`
- Onboarding layout reference: `Prompt Frontend/screen image/auth/Onboarding Screen.png`
- Splash layout reference: `Prompt Frontend/screen image/auth/Splash Screen.png`

## Core Structure to Create

### Constants (`lib/core/constants/`)
- `app_colors.dart` — primary, secondary, surface, error, success, payment status colors + **splash palette** (see `01-splash-prompt.md`)
- `app_strings.dart` — English keys (structure for Khmer later)
- `app_routes.dart` — route name constants
- `api_endpoints.dart` — path constants matching API v1
- `app_assets.dart` — `AppAssets.logoApp` → `assets/images/brand/logo_app.png`
...
| `app_logo.dart` | Official Vithey logo — `Image.asset(AppAssets.logoApp)` |
