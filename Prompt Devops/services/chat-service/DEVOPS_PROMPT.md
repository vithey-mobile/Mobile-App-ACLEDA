...
```text
vithey-backend/docker-compose.chat-service.yml
...
```bash
docker compose -f docker-compose.chat-service.yml up -d --build
curl http://localhost:8087/actuator/health
docker compose -f docker-compose.chat-service.yml down -v
```
...
CI must run Maven tests, build the Docker image with `SERVICE_PORT=8087`, and validate `docker-compose.chat-service.yml`.
